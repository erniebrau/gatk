...
public final class PileupElement {
    ...
    private CigarElement getNextIndelCigarElement() {
        if ( isBeforeDeletionStart() ) {
            ...
            Utils.validateState(element != null && element.getOperator() == CigarOperator.D, () -> "Immediately before deletion but the next cigar element isn't a deletion " + element);
            return element;
        } else if ( isBeforeInsertion() ) {
            final CigarElement element = getBetweenNextPosition().get(0);
            Utils.validateState(element.getOperator() == CigarOperator.I, () -> "Immediately before insertion but the next cigar element isn't an insertion " + element);
            return element;
        } else {
            return null;
        }

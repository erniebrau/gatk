...
public final class GATKVariantContextUtils {
    ...
    public static VariantContext simpleMerge(final Collection<VariantContext> unsortedVCs,
        ...
        for ( final VariantContext vc : VCs ) {
            Utils.validateState(longestVC.getStart() == vc.getStart(), () -> "BUG: attempting to merge VariantContexts with different start sites: first="+ first.toString() + " second=" + vc.toString());
    ...
    private static Map<Allele, Allele> createAlleleMapping(final Allele refAllele,
        ...
        Utils.validateState(refAllele.length() > myRef.length(), () -> "BUG: myRef="+myRef+" is longer than refAllele="+refAllele);
    ...
    public static List<VariantContext> splitIntoPrimitiveAlleles(final VariantContext vc) {
        ...
        Utils.validateState(ref.length == alt.length, "ref and alt alleles for MNP have different lengths");

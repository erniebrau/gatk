...
public abstract class PairHMM implements Closeable{
    ...
    double computeReadLikelihoodGivenHaplotypeLog10( final byte[] haplotypeBases,
        ...
        Utils.validateState(initialized, "Must call initialize before calling computeReadLikelihoodGivenHaplotypeLog10");
        ...
        Utils.validateState(result <= 0.0, () -> "PairHMM Log Probability cannot be greater than 0: " + String.format("haplotype: %s, read: %s, result: %f, PairHMM: %s", new String(haplotypeBases), new String(readBases), result, this.getClass().getSimpleName()));
        Utils.validateState(MathUtils.goodLog10Probability(result), () -> "Invalid Log Probability: " + result);
